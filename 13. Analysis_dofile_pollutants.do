...
prehead("\begin{table}\begin{center}\caption{The Effect of Weighted Annual Mean of PM 2.5 on Earnings Management}\label{tab: table26}\tabcolsep=0.1cm\scalebox{0.8}{\begin{tabular}{lcccc}\toprule")  ///
posthead("\midrule\\") ///
postfoot("\bottomrule\end{tabular}}\end{center}\footnotesize{Notes: The dependent variables are indicated at the top of each column. A description of all variables can be found in Table \ref{tab: variabledescriptions}. The dependent variables in columns (1)-(2) are: a firm's accrual earnings management, and the rank of the firm's accrual earnings management, respectively. The dependent variables in columns (3)-(4) are: a firm's real earnings management, and the rank of the firm's real earnings management, respectively. The regressor is the weighted average level of annual PM 2.5 for each city and year. Year fixed effects and industry fixed effects are included in all regressions. The same firm control variables are included as in Table \ref{tab: table4}. Standard errors are heteroskedastic-robust. *** p < 1\%, ** p < 5\%, * p < 10\%.}\end{table}") 
